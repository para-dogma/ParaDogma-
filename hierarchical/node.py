...
* **DogmaCompressor** -- semantic compression of responses.
* **DogmaNode** -- transport layer for inter-agent packet delivery.
...
class DogmaCompressor:
    ...
    def compress(self, text: str) -> str:
        ...
        logger.debug("Compressed %d -> %d chars", len(text), len(compressed))
        return compressed
...
class HierarchicalAgent:
    """Base agent integrating Compressor, Transport and Skill Library.

    Subclasses override ``execute(task)`` to provide role-specific logic.
    """
    ...
    def receive_task(self, task: Task) -> Result:
        """Accept task, enrich, execute, compress and return result."""
